...
class HostmeoutSms{
    ...
    public function getSender(){
        $settings = $this->getSettings();
        if(!$settings['api']){
            $this->addError("A valid API is not selected");
            $this->addLog("A valid API is not selected");
            return false;
        }else{
            return $settings['api'];
        }
    ...
    public function getSettings(){
        $result = select_query("mod_hostmeoutsms_settings", "*");
    ...
    function send(){
	    ...
	    $this->addLog("SenderID: ".$senderid);
            $this->addLog("SenderClass: ".$sender_function);

            include_once("senders/".$sender_function.".php");
            $sender = new $sender_function(trim($message),$this->getGsmnumber(),$senderid);
            $result = $sender->send();
    ...
    function getReport($msgid){
        $result = mysql_query("SELECT sender FROM mod_hostmeoutsms_messages WHERE msgid = '$msgid' LIMIT 1");
    ...
    function saveToDb($msgid,$status,$errors = null,$logs = null){
        ...
        $table = "mod_hostmeoutsms_messages";
        $values = array(
        ...
        $this->addLog("Messages has been saved to the database");
    }
    ...
    {
        $res = '<pre><p><i class="fa fa-info-circle"></i> <strong>Debug Result</strong><ul>';
    ...
    function checkHooks($hooks = null){
        ...
        foreach($hooks as $hook){
            $sql = "SELECT `id` FROM `mod_hostmeoutsms_templates` WHERE `name` = '".$hook['function']."' AND `type` = '".$hook['type']."' LIMIT 1";
            ...
            if($num_rows == 0){
                if($hook['type']){
                    ...
                    insert_query("mod_hostmeoutsms_templates", $values);
                    $i++;
                }
            }else{
                $values = array(
                ...
                update_query("mod_hostmeoutsms_templates", $values, "name = '" . $hook['name']."'");
            }
        }
        return $i;
    }
