<?php
class websmsplaza extends HostmeoutSms {
    ...
    function send(){
        ...
        $result = @file_get_contents('http://websmsplaza.com/components/com_spc/smsapi.php?username='.$params->user.'&password='.$params->pass.'&sender='.urlencode($senderid).'&recipient='.urlencode($this->gsmnumber).'&message='.urlencode($this->message).'&');
        $result = explode(" ",$result);

        if($result[0] == "OK") {
            $log[] = ("$result[0]! Message sent.");
    ...
    function balance(){
        ...
        if($params->user && $params->pass) {
            $result = @file_get_contents('http://websmsplaza.com/components/com_spc/smsapi.php?username='.$params->user.'&password='.$params->pass.'&balance=true&');
